...
def main():
    ...
    for t in range(num_steps):
        ...
        # only branch from longest paths
        path_lens = np.array(list(map(len, paths)))
        parents = np.flatnonzero(path_lens == path_lens.max())
